...
public class JtDatServiceImpl extends BaseServiceImpl<JtDatMapper, JtDat> implements JtDatService {
	...
	private BankIncomeDatService bankdatService;
	...
	public void importTax(MultipartFile file, String type, String openDate) throws Exception {
		...
		for (TaxExcel taxExcel : excels) {
			...
			CamsCompany company = camsCompanyService
					.selectOne(new EntityWrapper<CamsCompany>().eq("company_name", taxExcel.getCompanyName()));
			...
			if (!StringUtil.isEmpty(taxExcel.getChenJianShui()) && !taxExcel.getChenJianShui().equals("0")
					&& !taxExcel.getChenJianShui().equals("0.0") && !taxExcel.getChenJianShui().equals("暂无核税种")
					&& !taxExcel.getChenJianShui().equals("（自主有税申报）") && !taxExcel.getChenJianShui().equals("免税")) {
				map.put(SubjectEnum.SHUI_JIN_2171_08, taxExcel.getChenJianShui());
			}
			if (!StringUtil.isEmpty(taxExcel.getDiFangJiaoYuFuJiaShui())
					&& !taxExcel.getDiFangJiaoYuFuJiaShui().equals("0")
					&& !taxExcel.getDiFangJiaoYuFuJiaShui().equals("0.0")
					&& !taxExcel.getDiFangJiaoYuFuJiaShui().equals("暂无核税种")
					&& !taxExcel.getDiFangJiaoYuFuJiaShui().equals("（自主有税申报）")
					&& !taxExcel.getDiFangJiaoYuFuJiaShui().equals("免税")) {
				map.put(SubjectEnum.SHUI_JIN_2171_14, taxExcel.getDiFangJiaoYuFuJiaShui());
			}
			if (!StringUtil.isEmpty(taxExcel.getJiaoYuFuJiaShui()) && !taxExcel.getJiaoYuFuJiaShui().equals("0")
					&& !taxExcel.getJiaoYuFuJiaShui().equals("0.0") && !taxExcel.getJiaoYuFuJiaShui().equals("暂无核税种")
					&& !taxExcel.getJiaoYuFuJiaShui().equals("（自主有税申报）")
					&& !taxExcel.getJiaoYuFuJiaShui().equals("免税")) {
				map.put(SubjectEnum.SHUI_JIN_2171_13, taxExcel.getJiaoYuFuJiaShui());
			}
			if (!StringUtil.isEmpty(taxExcel.getYinHuaShui()) && !taxExcel.getYinHuaShui().equals("0")
					&& !taxExcel.getYinHuaShui().equals("0.0") && !taxExcel.getYinHuaShui().equals("暂无核税种")
					&& !taxExcel.getYinHuaShui().equals("（自主有税申报）") && !taxExcel.getYinHuaShui().equals("免税")) {
			...
			if (!StringUtil.isEmpty(taxExcel.getQiYeSuoDeShui()) && !taxExcel.getQiYeSuoDeShui().equals("0")
					&& !taxExcel.getQiYeSuoDeShui().equals("0.0") && !taxExcel.getQiYeSuoDeShui().equals("查账")
					&& !taxExcel.getQiYeSuoDeShui().equals("暂无核税种") && !taxExcel.getQiYeSuoDeShui().equals("（自主有税申报）")
					&& !taxExcel.getQiYeSuoDeShui().equals("免税") && !taxExcel.getQiYeSuoDeShui().equals("核定")
					&& !taxExcel.getQiYeSuoDeShui().equals("总公司申报")) {
				map.put(SubjectEnum.SHUI_JIN_2171_06, taxExcel.getQiYeSuoDeShui());
			}
			if (!StringUtil.isEmpty(taxExcel.getXiaoFeiShui()) && !taxExcel.getXiaoFeiShui().equals("0")
					&& !taxExcel.getXiaoFeiShui().equals("0.0") && !taxExcel.getXiaoFeiShui().equals("查账")
					&& !taxExcel.getXiaoFeiShui().equals("暂无核税种") && !taxExcel.getXiaoFeiShui().equals("（自主有税申报）")
					&& !taxExcel.getXiaoFeiShui().equals("免税") && !taxExcel.getXiaoFeiShui().equals("核定")
					&& !taxExcel.getXiaoFeiShui().equals("总公司申报")) {
				map.put(SubjectEnum.SHUI_JIN_2171_04, taxExcel.getXiaoFeiShui());
			}
			...
			if (!StringUtil.isEmpty(taxExcel.getWenHuaShiYejianSheShui())
					&& !taxExcel.getWenHuaShiYejianSheShui().equals("0")
					&& !taxExcel.getWenHuaShiYejianSheShui().equals("0.0")
					&& !taxExcel.getWenHuaShiYejianSheShui().equals("暂无核税种")
					&& !taxExcel.getWenHuaShiYejianSheShui().equals("（自主有税申报）")
					&& !taxExcel.getWenHuaShiYejianSheShui().equals("免税")) {
				map.put(SubjectEnum.SHUI_JIN_2171_16, taxExcel.getWenHuaShiYejianSheShui());
			}

			if (!StringUtil.isEmpty(taxExcel.getChengZhenTuDiShui()) && !taxExcel.getChengZhenTuDiShui().equals("0")
					&& !taxExcel.getChengZhenTuDiShui().equals("0.0")
					&& !taxExcel.getChengZhenTuDiShui().equals("暂无核税种")
					&& !taxExcel.getChengZhenTuDiShui().equals("（自主有税申报）")
					&& !taxExcel.getChengZhenTuDiShui().equals("免税")) {
				map.put(SubjectEnum.SHUI_JIN_2171_10, taxExcel.getChengZhenTuDiShui());
			}

			if (!StringUtil.isEmpty(taxExcel.getFangChanShui()) && !taxExcel.getFangChanShui().equals("0")
					&& !taxExcel.getFangChanShui().equals("0.0") && !taxExcel.getFangChanShui().equals("暂无核税种")
					&& !taxExcel.getFangChanShui().equals("（自主有税申报）") && !taxExcel.getFangChanShui().equals("免税")) {
				map.put(SubjectEnum.SHUI_JIN_2171_09, taxExcel.getFangChanShui());
			}
			if (!StringUtil.isEmpty(taxExcel.getZengZhiShui()) && !taxExcel.getZengZhiShui().equals("0")
					&& !taxExcel.getZengZhiShui().equals("0.0") && !taxExcel.getZengZhiShui().equals("暂无核税种")
					&& !taxExcel.getZengZhiShui().equals("（自主有税申报）") && !taxExcel.getZengZhiShui().equals("免税")) {
				map.put(SubjectEnum.SHUI_JIN_2171_02, taxExcel.getZengZhiShui());
			}
			if (!StringUtil.isEmpty(taxExcel.getCanJiRenBaoZhangJin()) && !taxExcel.getCanJiRenBaoZhangJin().equals("0")
					&& !taxExcel.getCanJiRenBaoZhangJin().equals("0.0")
					&& !taxExcel.getCanJiRenBaoZhangJin().equals("暂无核税种")
					&& !taxExcel.getCanJiRenBaoZhangJin().equals("（自主有税申报）")
					&& !taxExcel.getCanJiRenBaoZhangJin().equals("免税")) {
			...
			// dat=bankdatService.selectOne(new
			// EntityWrapper<Bankdat>().eq("ping_zheng_ri_qi", openDate).eq("ke_mu_dai_ma",
			// SubjectEnum.SHUI_JIN_2171_12.getValue().toString()).eq("company_name",
			// company.getCompanyName()));
			// if(dat==null) {
			// throw new
			// ApplicationContextException("请先导入"+company.getCompanyName()+"的个人所得税");
			// }
			...
			for (Map.Entry<SubjectEnum, String> entry : map.entrySet()) {
				...
				if (dat1.getKeMuDaiMa().equals(SubjectEnum.SHUI_JIN_2171_06.getValue())
						|| dat1.getKeMuDaiMa().equals(SubjectEnum.SHUI_JIN_2171_10.getValue())
						|| dat1.getKeMuDaiMa().equals(SubjectEnum.SHUI_JIN_2171_09.getValue())
						|| dat1.getKeMuDaiMa().equals(SubjectEnum.SHUI_JIN_2171_16.getValue())) {
					continue;
				}

				// 增值税不用计提
				if (!dat1.getKeMuDaiMa().equals(SubjectEnum.SHUI_JIN_2171_02.getValue())
						&& !dat1.getKeMuDaiMa().equals(SubjectEnum.SHUI_JIN_5502_23.getValue())) {
			...
			i = addJt(dat1, map, SubjectEnum.SHUI_JIN_2171_06, i, openDate, pingZhengHao, type, company,
					tranferBankImcomelist);
			i = addJt(dat1, map, SubjectEnum.SHUI_JIN_2171_10, i, openDate, pingZhengHao, type, company,
					tranferBankImcomelist);
			i = addJt(dat1, map, SubjectEnum.SHUI_JIN_2171_09, i, openDate, pingZhengHao, type, company,
					tranferBankImcomelist);
			i = addJt(dat1, map, SubjectEnum.SHUI_JIN_2171_16, i, openDate, pingZhengHao, type, company,
					tranferBankImcomelist);
			// 计提税金转换下个月支付
			if (tranferBankImcomelist.size() > 0) {
				bankdatService.tranferJtTaxToZhiFu(tranferBankImcomelist);
			}
	 ...
	 */
	@Override
	public void addTaxJt(String localJine, String invoiceNumber, String openDate, String pingZhengHao, String type,
	...
	private int addJt(JtDat dat1, Map<SubjectEnum, String> map, SubjectEnum subjectEnum, Integer hangHao,
			String openDate, String pingZhengHao, String type, CamsCompany company, List<JtDat> tranferBankImcomelist)
			throws Exception {
	...
	public void addGeRenSuoDeShui(String companyName, BigDecimal tax, String openDate) throws Exception {
		...
		String pingZhengHao = bankdatService.getPingZhengHao(companyName,
				DateUtil.getMonth(DateUtil.getDate(lastPingZhengRiQi)));
	...
	public void importZiChan(MultipartFile file, String companyName, String openDate) throws Exception {
		...
		for (int i = 0; i < excels.size(); i++) {
			...
			if (excels.get(i).getFaPiaoHaoMa() != null) {
				...
				addZiChan(excels.get(i).getChanPinMingChen(), companyName, customerDat, excels.get(i).getShuLiang(),
						kaiPiaoRiQi, invoiceNumber, excels.get(i).getFaPiaoZhongLei(), excels.get(i).getHanShuiJine(),
						excels.get(i).getBuHanShuiJine(), excels.get(i).getShuie(), excels.get(i).getDanJia());
			}
		}
	...
	@Override
	public void addZiChan(String feeName, String companyName, CustomerDat customerDat, String shuLiang, String openDate,
			String faPiaoHao, String buyType, String hanShuiJine, String buHanShuiJine, String shuie, String danJia)
			throws Exception {
		String pingZhengHao = getPingZhengHao(companyName, openDate);
	...
	@Override
	public void saveJtDat(List<JtDat> addDats, String openDate, String companyName, String customerCode,
			String jtType) {
		int month = DateUtil.getMonth(DateUtil.getDate(openDate));
		String pingZhengHao = getPingZhengHao(companyName, openDate);
		JtDat jtDat = null;
		String uuid = UUID.randomUUID().toString();
		for (JtDat dat : addDats) {
			jtDat = new JtDat();
			jtDat.setUuid(uuid);
			jtDat.setPingZhengRiQi(openDate);
			jtDat.setPingZhengZi("记");
			jtDat.setPingZhengHao(pingZhengHao);
			jtDat.setJtType(jtType);
			jtDat.setZhaiYao(dat.getZhaiYao());
			jtDat.setKeMuDaiMa(dat.getKeMuDaiMa());
			jtDat.setHuoBiDaiMa("RMB");
			jtDat.setHuiLv("1");
			jtDat.setShuLiang("0");
			jtDat.setDanJia("0");
			jtDat.setZhiDanRen("系统主管");
			jtDat.setShenHeRen("");
			jtDat.setGuoZhangRen("");
			jtDat.setFuDanJuShu("0");
			jtDat.setShiFouYiGuoZhang("0");
			jtDat.setMoBan("11记账凭证");
			jtDat.setCustomerCode(dat.getDanWei());
			jtDat.setBuMen("");
			jtDat.setYuanGong("");
			jtDat.setTongJi("");
			jtDat.setXiangMu("");
			jtDat.setFuKuanFangFa("");
			jtDat.setPiaoJuHao("");
			jtDat.setYuanBiFuKuanJinE("0");
			jtDat.setJtType(jtType);
			jtDat.setPingZhengLaiYuan("1");
			jtDat.setLaiYuanPingZheng(",,,");
			jtDat.setDaiDaYin("");
			jtDat.setZuoFeiBiaoZhi("0");
			jtDat.setCuoWuBiaoZhi("0");
			jtDat.setPingZhengCeHao("00");
			jtDat.setChuNaRen("");
			jtDat.setCompanyName(companyName);
			jtDat.setDeductionType("1");
			jtDat.setInvoiceNumber(dat.getInvoiceNumber());
			jtDat.setQiJian(String.valueOf(month));
			jtDat.setCreateTime(new Date());

			jtDat.setKeMuDaiMa(dat.getKeMuDaiMa());
			jtDat.setLocalAmount(dat.getLocalAmount());
			jtDat.setBorrowAmount(dat.getBorrowAmount());
			jtDat.setAlmsAmount(dat.getAlmsAmount());
			jtDat.setHangHao(dat.getHangHao());
			jtDat.setDanWei(customerCode);
			jtDat.setCustomerCode(customerCode);
			jtDat.setDeductionType("1");
			insert(jtDat);
