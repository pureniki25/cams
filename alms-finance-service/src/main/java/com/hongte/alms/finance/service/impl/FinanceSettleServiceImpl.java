...
public class FinanceSettleServiceImpl implements FinanceSettleService {
	...
	public List<CurrPeriodProjDetailVO> financeSettle(FinanceSettleReq financeSettleReq) {
		...
		if (!StringUtil.isEmpty(financeSettleReq.getRepayDate())) {
			financeSettleBaseDto.setFactRepayDate(DateUtil.getDate(financeSettleReq.getRepayDate()));
		}
		...
		for (RepaymentBizPlanSettleDto repaymentBizPlanSettleDto : currentPeriods) {
			...
			for (RepaymentBizPlanListDto beforePlanList : repaymentBizPlanSettleDto.getBeforeBizPlanListDtos()) {
				if (beforePlanList.getRepaymentBizPlanList().getRepayStatus() != null
						&& !beforePlanList.getRepaymentBizPlanList().getRepayStatus()
								.equals(SectionRepayStatusEnum.ONLINE_REPAID.getKey())
						&& !beforePlanList.getRepaymentBizPlanList().getRepayStatus()
								.equals(SectionRepayStatusEnum.ALL_REPAID.getKey())) {
	 ...
	 *
	 * @param dto
	 * @author 王继光 2018年7月17日 下午3:10:03
	 */
	private void calcSurplus(FinanceSettleBaseDto financeSettleBaseDto) {
		...
		if (fact.compareTo(plan) > 0) {
			...
			financeSettleBaseDto.getCurrPeriodProjDetailVOList().get(0)
					.setSurplus(financeSettleBaseDto.getSurplusAmount());
		}

		if (financeSettleBaseDto.getSurplusAmount() != null
				&& financeSettleBaseDto.getSurplusAmount().compareTo(BigDecimal.ZERO) > 0) {
			...
			accountantOverRepayLog.setRemark(String.format("收入于%s的%s期线下财务确认", financeSettleBaseDto.getBusinessId(),
					financeSettleBaseDto.getAfterId()));
	 ...
	 * 创建还款日志
	 * 
	 * @author 王继光 2018年8月3日 下午3:15:04
	...
	private void createConfirmLog(FinanceSettleBaseDto financeSettleBaseDto) {
		...
		repaymentConfirmLog.setPeriod(financeSettleBaseDto.getCurrentPeriods().get(0).getCurrBizPlanListDto()
				.getRepaymentBizPlanList().getPeriod());
		...
		if (financeSettleBaseDto.getFactRepayDate() != null) {
			repaymentConfirmLog.setRepayDate(financeSettleBaseDto.getFactRepayDate());
		}
		financeSettleBaseDto.setRepaymentConfirmLog(repaymentConfirmLog);
		/*
		 * if (!financeSettleBaseDto.getPreview()) { repaymentConfirmLog.insert(); }
		 */
	...
	public void makeRepaymentPlanAllPlan(FinanceSettleBaseDto financeSettleBaseDto, FinanceSettleReq financeSettleReq) {
		...
		// 查出当前还款计划的当前期
		RepaymentBizPlanList repaymentBizPlanList = bizPlanListService.selectOne(
		...
		if (repaymentBizPlanList != null) {
			...
			for (RepaymentBizPlanSettleDto repaymentBizPlanSettleDto : curPeriods) {
				List<RepaymentProjPlanSettleDto> repaymentProjPlanSettleDtos = repaymentBizPlanSettleDto
						.getProjPlanStteleDtos();

				for (RepaymentProjPlanSettleDto repaymentProjPlanSettleDto : repaymentProjPlanSettleDtos) {
					RepaymentProjPlanSettleDto projPlanSettleDto = projPlanSettleDtoMap
							.get(repaymentProjPlanSettleDto.getTuandaiProjectInfo().getProjectId());
					if (projPlanSettleDto != null) {
						logger.error("此业务有两个以上相同的标的还款计划 projPlanSettleDto" + JSON.toJSONString(projPlanSettleDto));
						throw new SettleRepaymentExcepiton("此业务有两个以上相同的标的还款计划",
								ExceptionCodeEnum.TOW_PROJ_PLAN.getValue().toString());
					}
					projPlanSettleDtoMap.put(repaymentProjPlanSettleDto.getTuandaiProjectInfo().getProjectId(),
							repaymentProjPlanSettleDto);
			...
			for (SettleFeesVO derateFee : financeSettleBaseDto.getDerates()) {
				for (RepaymentProjPlanSettleDto repaymentProjPlanSettleDto : projPlanSettleDtoList) {
					List<PlanListDetailShowPayDto> planListDetailShowPayDtos = sortFeeByShareProfitIndex(
							repaymentProjPlanSettleDto);
					for (PlanListDetailShowPayDto planListDetailShowPayDto : planListDetailShowPayDtos) {
						if (derateFee.getFeeId().equals(planListDetailShowPayDto.getFeelId())
							...
							if (derate.compareTo(BigDecimal.ZERO) == 0) {
								...
								financeSettleBaseDto.setCuralResource(financeSettleBaseDto.getRepaymentResources()
										.get(financeSettleBaseDto.getResourceIndex()));
							}
						}
					}
				}
			...
			for (RepaymentProjPlanSettleDto repaymentProjPlanSettleDto : projPlanSettleDtoList) {
				...
				for (PlanListDetailShowPayDto planListDetailShowPayDto : planListDetailShowPayDtos) {
					RepaymentProjPlanListDetail repaymentProjPlanListDetail = repaymentProjPlanSettleDto
							.getCurProjListDetailMap().get(planListDetailShowPayDto.getFeelId());
					...
					if (planListDetailShowPayDto.getShareProfitIndex() >= Constant.ONLINE_OFFLINE_FEE_BOUNDARY
							|| planListDetailShowPayDto.getFeelId()
					...
					if (financeSettleBaseDto.getCuralDivideAmount()
							.compareTo(planListDetailShowPayDto.getShowPayMoney()) > 0) {
						/* 足够还掉某一个费用项 */
						financeSettleBaseDto.setCuralDivideAmount(financeSettleBaseDto.getCuralDivideAmount()
								.subtract(planListDetailShowPayDto.getShowPayMoney()));
						createProjFactRepay(planListDetailShowPayDto.getShowPayMoney(), repaymentProjPlanListDetail,
						...
						if (financeSettleBaseDto.getCuralDivideAmount().compareTo(BigDecimal.ZERO) > 0) {
							createProjFactRepay(financeSettleBaseDto.getCuralDivideAmount(),
									repaymentProjPlanListDetail, financeSettleBaseDto.getCuralResource(),
									financeSettleBaseDto);
						}
						financeSettleBaseDto.setCuralDivideAmount(BigDecimal.ZERO);
						changeRepaymentResources(planListDetailShowPayDto, repaymentProjPlanListDetail,
								financeSettleBaseDto);
					}
				}
			}
			...
			for (RepaymentProjPlanSettleDto repaymentProjPlanSettleDto : projPlanSettleDtoList) {
				...
				for (PlanListDetailShowPayDto planListDetailShowPayDto : planListDetailShowPayDtos) {
					RepaymentProjPlanListDetail repaymentProjPlanListDetail = repaymentProjPlanSettleDto
							.getCurProjListDetailMap().get(planListDetailShowPayDto.getFeelId());
					// if (planListDetailShowPayDto.getShareProfitIndex() >=
					// Constant.ONLINE_OFFLINE_FEE_BOUNDARY || repaymentProjPlanListDetail == null )
					// {
					// continue;
					// }
					/* 线上滞纳金的核销优先级调整为先核销其他标的的本金利息服务费后在核销线上滞纳金 2018-08-31 update 贷后二期优化问题 肖莹环 */
					if (!planListDetailShowPayDto.getFeelId()
							.equals(RepayPlanFeeTypeEnum.OVER_DUE_AMONT_ONLINE.getUuid())) {
						continue;
					}
					/* 线上滞纳金的核销优先级调整为先核销其他标的的本金利息服务费后在核销线上滞纳金 2018-08-31 update 贷后二期优化问题 肖莹环 */
			...
			penaltyFees.sort(new Comparator<SettleFeesVO>() {
				// ,PRINCIPAL_PENALTY(70,"本金违约金","9883ee39-4448-11e8-b4f8-0242ac110003",5)
				// //uuid与信贷 固定费用id 一致
				// ,SUB_COMPANY_PENALTY(70,"分公司月收服务费违约金","9e0a6fc1-4448-11e8-b4f8-0242ac110003",5)
				// //uuid与信贷 固定费用id 一致
				// ,PLAT_PENALTY(70,"平台服务费违约金","a0b666c2-4448-11e8-b4f8-0242ac110003",5) //
				// uuid与信贷 固定费用id 一致
				// 平台服务费违约金-分公司月收服务费违约金-本金违约金
				@Override
				public int compare(SettleFeesVO o1, SettleFeesVO o2) {
					return o1.getShareProfitIndex().compareTo(o2.getShareProfitIndex());
				}
			});
			...
			for (SettleFeesVO settleFeesVO : penaltyFees) {
				...
				for (RepaymentProjPlanSettleDto repaymentProjPlanSettleDto : projPlanSettleDtoList) {
					...
					if (settleFeesVO.getProjectId()
						...
						RepaymentProjPlanListDetail repaymentProjPlanListDetail = repaymentProjPlanSettleDto
								.getCurrProjPlanListDto().getProjPlanListDetails().get(0);
						...
						projPlanListDetail.setAccountStatus(30);
						...
						projPlanListDetail
								.setPlanItemName(RepayPlanFeeTypeEnum.feeIdOf(settleFeesVO.getFeeId()).getDesc());
						...
						if (financeSettleBaseDto.isNoMoney()) {
							...
							financeSettleBaseDto.getUnderfillFees().add(planListDetailShowPayDto);
							repaymentProjPlanSettleDto.getCurrProjPlanListDto().getProjPlanListDetails()
									.add(projPlanListDetail);
							continue;
						}
			...
			for (SettleFeesVO settleFeesVO : penaltyFees) {
				...
				for (RepaymentProjPlanSettleDto repaymentProjPlanSettleDto : projPlanSettleDtoList) {
					...
					if (settleFeesVO.getProjectId()
						...
						// RepaymentProjPlanListDetail projPlanListDetail = new
			...
			for (RepaymentProjPlanSettleDto repaymentProjPlanSettleDto : projPlanSettleDtoList) {
				...
				for (PlanListDetailShowPayDto planListDetailShowPayDto : planListDetailShowPayDtos) {
					RepaymentProjPlanListDetail projDetail = repaymentProjPlanSettleDto.getCurProjListDetailMap()
							.get(planListDetailShowPayDto.getFeelId());
					if (planListDetailShowPayDto.getShareProfitIndex() < Constant.ONLINE_OFFLINE_FEE_BOUNDARY) {
						continue;
					}
					...
					if (projDetail == null) {
						...
						projDetail.setCreatSysType(RepayPlanCreateSysEnum.ALMS.getValue());
						projDetail.setDueDate(new Date());
						projDetail.setFactRepayDate(financeSettleBaseDto.getRepaymentResources()
								.get(financeSettleBaseDto.getRepaymentResources().size() - 1).getRepayDate());
						projDetail.setFeeId(RepayPlanFeeTypeEnum.OVER_DUE_AMONT_UNDERLINE.getUuid());
						projDetail.setOrigBusinessId(financeSettleBaseDto.getOrgBusinessId());
						projDetail.setPeriod(repaymentProjPlanSettleDto.getCurrProjPlanListDto()
								.getRepaymentProjPlanList().getPeriod());

						projDetail.setPlanDetailId(UUID.randomUUID().toString());
						projDetail.setPlanItemName(RepayPlanFeeTypeEnum.LACK_FEE.getDesc());
						projDetail.setPlanItemType(RepayPlanFeeTypeEnum.OVER_DUE_AMONT_UNDERLINE.getValue());
						// projDetail.setFeeId(planListDetailShowPayDto.getFeelId());
						projDetail.setPlanListId(repaymentProjPlanSettleDto.getCurrProjPlanListDto()
								.getRepaymentProjPlanList().getPlanListId());
						projDetail.setProjFactAmount(BigDecimal.ZERO);
						projDetail.setProjPlanAmount(planListDetailShowPayDto.getShowPayMoney());
						projDetail.setProjPlanDetailId(UUID.randomUUID().toString());
						projDetail.setProjPlanListId(repaymentProjPlanSettleDto.getCurrProjPlanListDto()
								.getRepaymentProjPlanList().getProjPlanListId());
						projDetail.setShareProfitIndex(Constant.ONLINE_OFFLINE_FEE_BOUNDARY + 1);

						// projDetail.setProjPlanAmount(planListDetailShowPayDto.getShowPayMoney());
						// projDetail.setPlanItemName(planListDetailShowPayDto.getFeelName());
						// projDetail.setPlanItemType(RepayPlanFeeTypeEnum.OVER_DUE_AMONT.getValue());
						// projDetail.setPeriod(repaymentBizPlanList.getPeriod());
						// projDetail.setPlanListId(repaymentBizPlanList.getPlanListId());
						// projDetail.setProjPlanDetailId(null);
						// projDetail.setProjPlanListId(null);
						/* 往期少缴费用 */
					}
					...
					if (financeSettleBaseDto.isNoMoney()) {
						...
						financeSettleBaseDto.getUnderfillFees().add(planListDetailShowPayDto);
						repaymentProjPlanSettleDto.getCurrProjPlanListDto().getProjPlanListDetails().add(projDetail);
						continue;
					}
					...
					if (financeSettleBaseDto.getCuralDivideAmount()
						...
						financeSettleBaseDto.setCuralDivideAmount(financeSettleBaseDto.getCuralDivideAmount()
								.subtract(planListDetailShowPayDto.getShowPayMoney()));
						createProjFactRepay(planListDetailShowPayDto.getShowPayMoney(), projDetail,
								financeSettleBaseDto.getCuralResource(), financeSettleBaseDto);
					} else if (financeSettleBaseDto.getCuralDivideAmount()
							.compareTo(planListDetailShowPayDto.getShowPayMoney()) == 0) {
						financeSettleBaseDto.setCuralDivideAmount(financeSettleBaseDto.getCuralDivideAmount()
						...
						createProjFactRepay(planListDetailShowPayDto.getShowPayMoney(), projDetail,
							...
							.compareTo(planListDetailShowPayDto.getShowPayMoney()) < 0) {
						if (financeSettleBaseDto.getCuralDivideAmount().compareTo(BigDecimal.ZERO) > 0) {
							createProjFactRepay(financeSettleBaseDto.getCuralDivideAmount(), projDetail,
									financeSettleBaseDto.getCuralResource(), financeSettleBaseDto);
						}
						financeSettleBaseDto.setCuralDivideAmount(BigDecimal.ZERO);
						changeRepaymentResources(planListDetailShowPayDto, projDetail, financeSettleBaseDto);
					}
					repaymentProjPlanSettleDto.getCurrProjPlanListDto().getProjPlanListDetails().add(projDetail);
				}
			...
			for (SettleFeesVO settleFeesVO : financeSettleBaseDto.getSettleInfoVO().getOtherFees()) {
				...
				RepaymentProjPlanListDetail repaymentProjPlanListDetail = masterProject.getCurrProjPlanListDto()
						.getProjPlanListDetails().get(0);
				...
				projPlanListDetail.setAccountStatus(20);
				projPlanListDetail
						.setPeriod(masterProject.getCurrProjPlanListDto().getRepaymentProjPlanList().getPeriod());
				projPlanListDetail.setPlanListId(
						masterProject.getCurrProjPlanListDto().getRepaymentProjPlanList().getPlanListId());
				projPlanListDetail.setProjPlanListId(
						masterProject.getCurrProjPlanListDto().getRepaymentProjPlanList().getProjPlanListId());
				projPlanListDetail.setProjPlanDetailId(UUID.randomUUID().toString());
			...
			if (!financeSettleBaseDto.getPreview()) {
				...
				if (!CollectionUtils.isEmpty(financeSettleBaseDto.getUnderfillFees())) {
					...
					for (PlanListDetailShowPayDto planListDetailShowPayDto : financeSettleBaseDto.getUnderfillFees()) {
						if (planListDetailShowPayDto.getShareProfitIndex() != null && planListDetailShowPayDto
								.getShareProfitIndex() < Constant.ONLINE_OFFLINE_FEE_BOUNDARY) {
				...
				for (RepaymentBizPlanSettleDto bizPlanSettleDto : curPeriods) {
					...
					for (RepaymentProjPlanSettleDto repaymentProjPlanSettleDto : bizPlanSettleDto
							.getProjPlanStteleDtos()) {

						RepayPlanSettleStatusEnum pe = setProjPlanList(financeSettleBaseDto,
								repaymentProjPlanSettleDto);
						/* 更新标PLAN */
						...
						/* 将标的细项回填业务细项 */
						if (repaymentProjPlanSettleDto.getCurFactRepayAmount() != null) {
							for (Entry<String, BigDecimal> entry : repaymentProjPlanSettleDto.getCurFactRepayAmount()
									.entrySet()) {
								for (RepaymentBizPlanListDetail bizPlanListDetail : bizPlanSettleDto
										.getCurrBizPlanListDto().getBizPlanListDetails()) {
									if (bizPlanListDetail.getPlanItemType()
											.equals(RepayPlanFeeTypeEnum.PENALTY_AMONT.getValue())
											|| bizPlanListDetail.getPlanItemType()
													.equals(RepayPlanFeeTypeEnum.OTHER_FEE.getValue())
											|| bizPlanListDetail.getPlanItemName()
													.equals(RepayPlanFeeTypeEnum.LACK_FEE.getDesc())) {
										continue;
									}
									if (bizPlanListDetail.getFeeId().equals(entry.getKey())) {
										...
										bizPlanListDetail
												.setFactAmount(bizPlanListDetail.getFactAmount().add(entry.getValue()));
									}
								}
							}
						}
						...
						/* 新增提前违约金细项||其他费用||往期少缴 */
						for (RepaymentProjPlanListDetail repaymentProjPlanListDetail : repaymentProjPlanSettleDto
								.getCurrProjPlanListDto().getProjPlanListDetails()) {
							if (repaymentProjPlanListDetail.getPlanItemType()
									.equals(RepayPlanFeeTypeEnum.PENALTY_AMONT.getValue())
									|| repaymentProjPlanListDetail.getPlanItemType()
											.equals(RepayPlanFeeTypeEnum.OTHER_FEE.getValue())
									|| repaymentProjPlanListDetail.getPlanItemName()
											.equals(RepayPlanFeeTypeEnum.LACK_FEE.getDesc())) {
								RepaymentBizPlanListDetail bizPlanDetail = existBizPlanDetail(
										bizPlanSettleDto.getCurrBizPlanListDto().getBizPlanListDetails(),
										repaymentProjPlanListDetail.getPlanDetailId());
								if (bizPlanDetail == null) {
									bizPlanDetail = new RepaymentBizPlanListDetail();
									RepaymentBizPlanListDetail bizPlanListDetail = bizPlanSettleDto
											.getCurrBizPlanListDto().getBizPlanListDetails().get(0);
									...
									bizPlanDetail
											.setShareProfitIndex(repaymentProjPlanListDetail.getShareProfitIndex());
								...
								bizPlanDetail.setPlanAmount(bizPlanDetail.getPlanAmount()
										.add(repaymentProjPlanListDetail.getProjPlanAmount()));
								bizPlanDetail.setFactAmount(bizPlanDetail.getFactAmount()
										.add(repaymentProjPlanListDetail.getProjFactAmount()));
						...
						for (RepaymentProjPlanListDetail projDetail : repaymentProjPlanSettleDto
								.getCurrProjPlanListDto().getProjPlanListDetails()) {
							planAmount = planAmount.add(projDetail.getProjPlanAmount());
							factReapy = factReapy.add(projDetail.getProjFactAmount() == null ? BigDecimal.ZERO
									: projDetail.getProjFactAmount());
						...
						repaymentProjPlanSettleDto.getCurrProjPlanListDto().getRepaymentProjPlanList()
								.setFactRepayDate(financeSettleBaseDto.getRepaymentConfirmLog().getRepayDate());
						repaymentProjPlanSettleDto.getCurrProjPlanListDto().getRepaymentProjPlanList()
								.setCurrentStatus(RepayCurrentStatusEnums.已还款.toString());
						repaymentProjPlanSettleDto.getCurrProjPlanListDto().getRepaymentProjPlanList()
								.setRepayFlag(PepayPlanRepayFlagStatusEnum.UNDERLINE_ALL_SETTLE.getValue());
						repaymentProjPlanSettleDto.getCurrProjPlanListDto().getRepaymentProjPlanList()
								.updateAllColumnById();

						/* 更新标的明细 */
						for (RepaymentProjPlanListDetail detail : repaymentProjPlanSettleDto.getCurrProjPlanListDto()
								.getProjPlanListDetails()) {
							if (detail.getProjPlanDetailId() != null && !detail.getPlanItemType()
									.equals(RepayPlanFeeTypeEnum.PENALTY_AMONT.getValue())) {
								detail.updateAllColumnById();
							}
							if (detail.getProjPlanDetailId() != null
									&& (detail.getPlanItemType().equals(RepayPlanFeeTypeEnum.PENALTY_AMONT.getValue())
											|| detail.getPlanItemType()
													.equals(RepayPlanFeeTypeEnum.OTHER_FEE.getValue()))
									|| detail.getPlanItemName().equals(RepayPlanFeeTypeEnum.LACK_FEE.getDesc())) {
						...
						/* 更新往后期的标的期数 */
						for (RepaymentProjPlanListDto afterProjPlanListDto : repaymentProjPlanSettleDto
								.getAfterProjPlanListDtos()) {
							afterProjPlanListDto.getRepaymentProjPlanList()
									.setCurrentStatus(RepayCurrentStatusEnums.已还款.toString());
							afterProjPlanListDto.getRepaymentProjPlanList()
									.setRepayFlag(PepayPlanRepayFlagStatusEnum.UNDERLINE_ALL_SETTLE.getValue());
							afterProjPlanListDto.getRepaymentProjPlanList()
						...
						projPlanList
								.add(repaymentProjPlanSettleDto.getCurrProjPlanListDto().getRepaymentProjPlanList());
					}
					...
					for (RepaymentBizPlanListDetail bizPlanListDetail : bizPlanSettleDto.getCurrBizPlanListDto()
							.getBizPlanListDetails()) {
						...
						if (bizPlanListDetail.getPlanItemType()
								.equals(RepayPlanFeeTypeEnum.OVER_DUE_AMONT.getValue())) {
							overDueFee = overDueFee.add(bizPlanListDetail.getPlanAmount());
						}
						// List<RepaymentProjFactRepay> bizPlanFactRepay =
						// calcBizPlanFactRepay(bizPlanSettleDto.getRepaymentBizPlan().getPlanId(),
						// bizPlanListDetail.getPlanDetailId(), financeSettleBaseDto);
						 ...
						 * bizPlanListDetail.setFactAmount((bizPlanListDetail.getFactAmount()==null?
						 * BigDecimal.ZERO:bizPlanListDetail.getFactAmount())
						 * .add(repaymentProjFactRepay.getFactAmount()));
						 */
						bizPlanListDetail.setFactRepayDate(financeSettleBaseDto.getRepaymentResources()
								.get(financeSettleBaseDto.getRepaymentResources().size() - 1).getRepayDate());
						if (bizPlanListDetail.getPlanItemType().equals(RepayPlanFeeTypeEnum.PENALTY_AMONT.getValue())
								|| bizPlanListDetail.getPlanItemType().equals(RepayPlanFeeTypeEnum.OTHER_FEE.getValue())
								|| bizPlanListDetail.getPlanItemName()
										.equals(RepayPlanFeeTypeEnum.LACK_FEE.getDesc())) {
					...
					if (factReapy.compareTo(BigDecimal.ZERO) > 0) {
						bizPlanSettleDto.getCurrBizPlanListDto().getRepaymentBizPlanList()
								.setRepayStatus(SectionRepayStatusEnum.SECTION_REPAID.getKey());
					}
					if (factReapy.compareTo(onLineFee) >= 0) {
						bizPlanSettleDto.getCurrBizPlanListDto().getRepaymentBizPlanList()
					...
					/* 更新业务planList */
					// bizPlanSettleDto.getCurrBizPlanListDto().getRepaymentBizPlanList().setTotalBorrowAmount(planAmount);
					// bizPlanSettleDto.getCurrBizPlanListDto().getRepaymentBizPlanList().setOverdueAmount(overDueFee);
					bizPlanSettleDto.getCurrBizPlanListDto().getRepaymentBizPlanList()
							.setFactRepayDate(financeSettleBaseDto.getRepaymentConfirmLog().getRepayDate());
					bizPlanSettleDto.getCurrBizPlanListDto().getRepaymentBizPlanList()
							.setCurrentStatus(RepayCurrentStatusEnums.已还款.toString());
					bizPlanSettleDto.getCurrBizPlanListDto().getRepaymentBizPlanList()
							.setRepayFlag(PepayPlanRepayFlagStatusEnum.UNDERLINE_ALL_SETTLE.getValue());
					bizPlanSettleDto.getCurrBizPlanListDto().getRepaymentBizPlanList()
							.setFinanceComfirmDate(new Date());
					bizPlanSettleDto.getCurrBizPlanListDto().getRepaymentBizPlanList()
							.setFinanceConfirmUser(financeSettleBaseDto.getUserId());
					;
					bizPlanSettleDto.getCurrBizPlanListDto().getRepaymentBizPlanList()
							.setFinanceConfirmUserName(financeSettleBaseDto.getUserName());
					;
					...
					for (RepaymentBizPlanListDto bizPlanListDto : bizPlanSettleDto.getAfterBizPlanListDtos()) {
						bizPlanListDto.getRepaymentBizPlanList()
								.setCurrentStatus(RepayCurrentStatusEnums.已还款.toString());
						bizPlanListDto.getRepaymentBizPlanList()
								.setRepayFlag(PepayPlanRepayFlagStatusEnum.UNDERLINE_ALL_SETTLE.getValue());
						bizPlanListDto.getRepaymentBizPlanList()
				...
				for (RepaymentProjPlanList repaymentProjPlanList : projPlanList) {
					...
					log.setCreateUser(loginUserInfoHelper.getUserId() == null ? Constant.ADMIN_ID
			...
			throw new SettleRepaymentExcepiton("找不到对应的还款计划列表",
	 ...
	 * 在当前期找主借标
	 * 
	 * @author 王继光 2018年8月24日 上午10:33:45
	 * @param cur
	 * @return
	 */
	private RepaymentProjPlanSettleDto getMasterProject(List<RepaymentBizPlanSettleDto> cur) {
		for (RepaymentBizPlanSettleDto repaymentBizPlanSettleDto : cur) {
			for (RepaymentProjPlanSettleDto projPlanDto : repaymentBizPlanSettleDto.getProjPlanStteleDtos()) {
				if (projPlanDto.getTuandaiProjectInfo().getMasterIssueId()
						.equals(projPlanDto.getTuandaiProjectInfo().getProjectId())) {
					return projPlanDto;
				}
			}
		}
	 ...
	 * 根据planListDetail 查找
	 * 
	 * @author 王继光 2018年8月14日 下午9:40:58
	 * @param details
	 * @param planListDetailId
	 * @return
	 */
	private RepaymentBizPlanListDetail existBizPlanDetail(List<RepaymentBizPlanListDetail> details,
	 ...
	 * 根据实还 设置 标的状态
	 * 
	 * @author 王继光 2018年8月10日 下午10:43:26
	 * @param financeSettleBaseDto
	...
	private RepayPlanSettleStatusEnum setProjPlanList(FinanceSettleBaseDto financeSettleBaseDto,
		...
		for (Entry<String, PlanListDetailShowPayDto> et : repaymentProjPlanSettleDto.getCurShowPayFeels().entrySet()) {
			...
			if (repaymentProjPlanSettleDto.getCurFactRepayAmount() != null
					&& repaymentProjPlanSettleDto.getCurFactRepayAmount().containsKey(feeId)) {
	 ...
	 * 计算业务层面的实还
	 * 
	 * @author 王继光 2018年7月30日 下午8:19:24
	 * @param planId
	 * @param planListDetailId
	 * @param dto
	 * @return
	 */
	private List<RepaymentProjFactRepay> calcBizPlanFactRepay(String planId, String planListDetailId,
			FinanceSettleBaseDto dto) {
	 ...
	 * 取下个还款来源
	 * 
	 * @author 王继光 2018年7月30日 上午11:53:32
	 * @param financeSettleBaseDto
	 * @param noMoney
	 * @return
	 */
	private boolean changeRepaymentResources(FinanceSettleBaseDto financeSettleBaseDto) {
		/* 改还款来源继续往下走 */
		try {
			financeSettleBaseDto.setCuralResource(
					financeSettleBaseDto.getRepaymentResources().get(financeSettleBaseDto.getResourceIndex() + 1));
			financeSettleBaseDto.setCuralDivideAmount(financeSettleBaseDto.getCuralResource().getRepayAmount());
			financeSettleBaseDto.setResourceIndex(financeSettleBaseDto.getResourceIndex() + 1);
			financeSettleBaseDto.setNoMoney(false);
			return true;
	 ...
	 * 
	 * @author 王继光 2018年7月30日 下午3:39:47
	 * @param projListDetail
	 * @param financeSettleBaseDto
	 * @param noMoney
	 */
	private void changeRepaymentResources(PlanListDetailShowPayDto fee, RepaymentProjPlanListDetail projListDetail,
			FinanceSettleBaseDto financeSettleBaseDto) {
		...
		if (changeRepaymentResources(financeSettleBaseDto)) {
			if (financeSettleBaseDto.getCuralDivideAmount().compareTo(unpaid) >= 0) {
				...
				createProjFactRepay(unpaid, projListDetail, financeSettleBaseDto.getCuralResource(),
						financeSettleBaseDto);
				...
				// projListDetail.setFactRepayDate(financeSettleBaseDto.getCuralResource().getRepayDate());
				createProjFactRepay(financeSettleBaseDto.getCuralDivideAmount(), projListDetail,
						financeSettleBaseDto.getCuralResource(), financeSettleBaseDto);
				financeSettleBaseDto.setCuralDivideAmount(financeSettleBaseDto.getCuralDivideAmount()
						.subtract(financeSettleBaseDto.getCuralDivideAmount()));
			...
			// createProjFactRepay(BigDecimal.ZERO, projListDetail,
			// financeSettleBaseDto.getCuralResource(), financeSettleBaseDto);
			/* 没钱也要创建一条实还为零的,2018-10-22 因坏账结清资金分发需要 特此修改 */
		}
	 ...
	 * 
	 * @author 王继光 2018年7月30日 下午12:01:49
	 * @param repaymentProjPlanSettleDto
	 * @return
	 */
	private List<PlanListDetailShowPayDto> sortFeeByShareProfitIndex(
			RepaymentProjPlanSettleDto repaymentProjPlanSettleDto) {
		List<PlanListDetailShowPayDto> planListDetailShowPayDtos = new ArrayList<>();
		for (Entry<String, PlanListDetailShowPayDto> entry : repaymentProjPlanSettleDto.getCurShowPayFeels()
				.entrySet()) {
	...
	public void sumProjMoney(CurrPeriodProjDetailVO vo, RepaymentSettleMoneyDto moneyDto) {
		...
		vo.setTotal(vo.getItem10().add(vo.getItem20()).add(vo.getItem30()).add(vo.getItem50())
				.add(vo.getOfflineOverDue()).add(vo.getOnlineOverDue()));
	...
	public void repayLog(FinanceSettleBaseDto financeSettleBaseDto) {
		...
		if (CollectionUtils.isNotEmpty(ptojPlanList)) {
			...
			for (RepaymentProjPlanList projPlanList : ptojPlanList) {
				...
				log.setCreateTime(new Date());
				log.setCreateUser(
						loginUserInfoHelper.getUserId() == null ? Constant.ADMIN_ID : loginUserInfoHelper.getUserId());
				repaymentConfirmPlatRepayLogService.insert(log);
			}
	...
	public void createSettleLogDetail(BigDecimal amount, RepaymentSettleMoneyDto moneyDto, RepaymentResource resource,
			FinanceSettleBaseDto financeSettleBaseDto) {
		...
		if (projPlanId != null) {
			...
			// 用户扣款详情回填dto
			// List<RepaymentSettleLogDetail> repaymentSettleLogDetailLists =
			// financeSettleBaseDto.getRepaymentSettleLogDetailList();
	 ...
	 * 处理还款来源
	 * 
	 * @author 王继光 2018年8月3日 下午2:40:26
	 * @param financeSettleBaseDto
	...
	public void handleRepaymentResource(FinanceSettleBaseDto financeSettleBaseDto, FinanceSettleReq financeSettleReq) {
		...
		if (!CollectionUtils.isEmpty(mprIds)) {
			List<MoneyPoolRepayment> moneyPoolRepaymentList = moneyPoolRepaymentMapper
					.selectList(new EntityWrapper<MoneyPoolRepayment>().in("id", mprIds));
			for (MoneyPoolRepayment moneyPoolRepayment : moneyPoolRepaymentList) {
				...
				repaymentResource.setConfirmLogId(financeSettleBaseDto.getUuid());
				repaymentResources.add(repaymentResource);
				financeSettleBaseDto.setRepayFactAmount(
						financeSettleBaseDto.getRepayFactAmount().add(repaymentResource.getRepayAmount()));
				...
				if (!financeSettleReq.getPreview()) {
					...
					if (financeSettleBaseDto.getRepaymentConfirmLog().getRepayDate() == null) {
						financeSettleBaseDto.getRepaymentConfirmLog().setRepayDate(repaymentResource.getRepayDate());
					}

					List<MoneyPoolRepayment> moneyPoolRepayments = moneyPoolRepaymentMapper
							.selectBatchIds(financeSettleReq.getMprIds());
		...
		if (surplus != null && surplus.compareTo(new BigDecimal("0")) > 0) {
			...
			accountantOverRepayLog.setRemark(
					String.format("支出于%s的%s期线下财务結清", financeSettleReq.getBusinessId(), financeSettleReq.getAfterId()));
			accountantOverRepayLog.setLogId(UUID.randomUUID().toString());
	...
	private RepaymentProjFactRepay createProjFactRepay(BigDecimal divideAmount, RepaymentProjPlanListDetail detail,
			RepaymentResource resource, FinanceSettleBaseDto financeSettleBaseDto) {
	...
	private void rendCurrPeriodProjDetailVO(BigDecimal amount, RepaymentProjPlanListDetail detail,
		...
		case 61:
			...
			if (detail.getFeeId().equals(RepayPlanFeeTypeEnum.OVER_DUE_AMONT_UNDERLINE.getUuid())
					|| detail.getFeeId().equals(RepayPlanFeeTypeEnum.LACK_FEE.getUuid())) {
				vo.setOfflineOverDue(vo.getOfflineOverDue().add(amount));
			}
			break;
		case 70:
	 ...
	 * @param fact
	 */
	private void addProjFactRepays(FinanceSettleBaseDto financeSettleBaseDto, String bizPlanListDetailId,
			RepaymentProjFactRepay fact) {
	 ...
	 * 将实还累加并保存进标的
	 * 
	 * @author 王继光 2018年8月10日 下午8:33:07
	 * @param financeSettleBaseDto
	 * @param fact
	 */
	...
	public SettleInfoVO settleInfoVO(FinanceSettleReq req) {
		...
		List<MoneyPoolRepayment> moneyPoolRepayments = moneyPoolRepaymentMapper
				.selectList(new EntityWrapper<MoneyPoolRepayment>().eq("plan_list_id", cur.getPlanListId())
						.eq("is_finance_match", 1).orderBy("account_money", false).last(" limit 1 "));
		...
		/*
		 * repaymentBizPlanListDetailMapper.selectLastPlanListDerateFees(req.
		 * getBusinessId(), cur.getDueDate(), req.getPlanId())
		 */
		infoVO.setDerates(listDerate(cur.getPlanListId()));
		infoVO.setLackFees(repaymentBizPlanListDetailMapper.selectLastPlanListLackFees(req.getBusinessId(),
				cur.getDueDate(), req.getPlanId()));
		...
		infoVO.setSubtotal(infoVO.getSubtotal().add(infoVO.getItem10()).add(infoVO.getItem20()).add(infoVO.getItem30())
				.add(infoVO.getItem50()));
		infoVO.setTotal(infoVO.getTotal().add(infoVO.getSubtotal()).add(infoVO.getOfflineOverDue())
				.add(infoVO.getOnlineOverDue()).subtract(infoVO.getDerate()).add(infoVO.getPlanRepayBalance())
				.add(infoVO.getPenalty()).add(infoVO.getOther()));
	 ...
	 * 
	 * @author 王继光 2018年8月7日 上午10:50:23
	 * @param planListId
	 * @return
	 */
	 ...
	 * @param planId
	 * @return
	 * @author 王继光 2018年7月11日 下午10:06:09
	 */
	private void calcPenalty(final FinanceSettleReq req, SettleInfoVO infoVO) {
		...
		BasicBusiness basicBusiness = basicBusinessMapper.selectById(req.getBusinessId());
		Integer shareProfitIndex = profitItemSetService.getLevel(basicBusiness.getBusinessType().toString(), 60,
				RepayPlanFeeTypeEnum.OVER_DUE_AMONT_UNDERLINE.getUuid()).get("feeLevel");
		
		for (RepaymentBizPlanList bizPlanList : getCurrenPeroids(req)) {

			RepaymentBizPlan bizPlan = repaymentBizPlanService
					.selectOne(new EntityWrapper<RepaymentBizPlan>().eq("plan_id", bizPlanList.getPlanId()));
			...
			List<RepaymentProjPlan> projPlans = repaymentProjPlanMapper
					.selectList(new EntityWrapper<RepaymentProjPlan>().in("proj_plan_id", projPlanStrs));
			...
			List<ProjExtRate> extRates = projExtRateMapper
					.selectList(new EntityWrapper<ProjExtRate>().eq("business_id", bizPlanList.getOrigBusinessId())
							.in("project_id", projectIds).eq("rate_type", RepayPlanFeeTypeEnum.PENALTY_AMONT.getValue())
			...
			for (ProjExtRate projExtRate : extRates) {
				...
				if (PepayPlanProjExtRatCalEnum.BY_BORROW_MONEY.getValue() == projExtRate.getCalcWay()) {
					...
					// 2剩余本金*费率值
					BigDecimal upaid = repaymentProjPlanMapper.sumProjectItem10Unpaid(projExtRate.getProjectId(),
							bizPlanList.getPlanId());
					...
					// 4 剩余的平台服务费合计
					penalty = penalty
							.add(repaymentProjPlanListDetailMapper.calcSurplusPlatformFees(bizPlanList.getBusinessId(),
									projExtRate.getProjectId(), bizPlanList.getPlanId(), bizPlanList.getPeriod()));
				} else if (PepayPlanProjExtRatCalEnum.BY_MONTH_COM_FEE.getValue() == projExtRate.getCalcWay()) {
					// 5 费率值*月收分公司服务费
					BigDecimal serviceFee = repaymentProjPlanListDetailMapper.calcService(bizPlanList.getBusinessId(),
							projExtRate.getProjectId(), bizPlanList.getPlanId(), bizPlanList.getPeriod());
					...
					// 6 费率值*月收平台服务费
					BigDecimal platformFee = repaymentProjPlanListDetailMapper.calcPlatFee(bizPlanList.getBusinessId(),
							projExtRate.getProjectId(), bizPlanList.getPlanId(), bizPlanList.getPeriod());
					...
					BigDecimal servicePenalty = projExtRateMapper.calcProjextRate(projExtRate.getProjectId(),
							RepayPlanFeeTypeEnum.PENALTY_AMONT.getValue().toString(),
							RepayPlanFeeTypeEnum.SUB_COMPANY_PENALTY.getUuid(), bizPlanList.getPeriod().toString());
					BigDecimal serviceFee = repaymentProjPlanListDetailMapper.calcService(bizPlanList.getBusinessId(),
							projExtRate.getProjectId(), bizPlanList.getPlanId(), bizPlanList.getPeriod());
					BigDecimal service = servicePenalty.multiply(serviceFee);

					BigDecimal platformPenalty = projExtRateMapper.calcProjextRate(projExtRate.getProjectId(),
							RepayPlanFeeTypeEnum.PENALTY_AMONT.getValue().toString(),
							RepayPlanFeeTypeEnum.PLAT_PENALTY.getUuid(), bizPlanList.getPeriod().toString());
					BigDecimal platformFee = repaymentProjPlanListDetailMapper.calcPlatFee(bizPlanList.getBusinessId(),
							projExtRate.getProjectId(), bizPlanList.getPlanId(), bizPlanList.getPeriod());
					...
					throw new ServiceRuntimeException(
							"错误： projExtRate.CalcWay[" + projExtRate.getCalcWay() + "]尚未有对应算法");
				}
				...
				if (RepayPlanFeeTypeEnum.PRINCIPAL_PENALTY.getUuid().equals(projExtRate.getFeeId())) {
					...
					fee.setShareProfitIndex(Constant.ONLINE_OFFLINE_FEE_BOUNDARY - 2);
				} else if (RepayPlanFeeTypeEnum.HT_PENALTY.getUuid().equals(projExtRate.getFeeId())) {
					/*提前还款违约金是：粤财违约金》鸿特违约金*/
					fee.setShareProfitIndex(shareProfitIndex+2);
				} else if (RepayPlanFeeTypeEnum.YC_PENALTY.getUuid().equals(projExtRate.getFeeId())) {
					fee.setShareProfitIndex(shareProfitIndex+1);
					/*提前还款违约金是：粤财违约金》鸿特违约金*/
				}
				/* 将违约金设为线上费用 */
				fees.add(fee);
	...
	private void calcCurPeriod(RepaymentBizPlanList repaymentBizPlanList, SettleInfoVO infoVO, Date factRepayDate,
		...
		if (StringUtil.isEmpty(req.getPlanId())) {
			...
			for (RepaymentBizPlanList repaymentBizPlanList2 : list) {
				BigDecimal item20 = repaymentProjPlanListDetailMapper
						.calcBizPlanListUnpaid(repaymentBizPlanList2.getPlanListId(), "20", null);
				BigDecimal item30 = repaymentProjPlanListDetailMapper
						.calcBizPlanListUnpaid(repaymentBizPlanList2.getPlanListId(), "30", null);
				BigDecimal item50 = repaymentProjPlanListDetailMapper
						.calcBizPlanListUnpaid(repaymentBizPlanList2.getPlanListId(), "50", null);
				...
				BigDecimal item60offline = repaymentProjPlanListDetailMapper.calcBizPlanListUnpaid(
						repaymentBizPlanList2.getPlanListId(), "60",
						RepayPlanFeeTypeEnum.OVER_DUE_AMONT_UNDERLINE.getUuid());
				BigDecimal item60online = repaymentProjPlanListDetailMapper.calcBizPlanListUnpaid(
						repaymentBizPlanList2.getPlanListId(), "60",
			...
			BigDecimal item20 = repaymentProjPlanListDetailMapper
					.calcBizPlanListUnpaid(repaymentBizPlanList.getPlanListId(), "20", null);
			BigDecimal item30 = repaymentProjPlanListDetailMapper
					.calcBizPlanListUnpaid(repaymentBizPlanList.getPlanListId(), "30", null);
			BigDecimal item50 = repaymentProjPlanListDetailMapper
					.calcBizPlanListUnpaid(repaymentBizPlanList.getPlanListId(), "50", null);
			...
			BigDecimal item60offline = repaymentProjPlanListDetailMapper.calcBizPlanListUnpaid(
					repaymentBizPlanList.getPlanListId(), "60",
					RepayPlanFeeTypeEnum.OVER_DUE_AMONT_UNDERLINE.getUuid());
			BigDecimal item60online = repaymentProjPlanListDetailMapper.calcBizPlanListUnpaid(
					repaymentBizPlanList.getPlanListId(), "60", RepayPlanFeeTypeEnum.OVER_DUE_AMONT_ONLINE.getUuid());
	 ...
	 * 更新备注
	 * 
	 * @author 王继光 2018年7月10日 下午3:18:25
	...
	private void updateRemark(FinanceSettleBaseDto financeBaseDto) {
		for (RepaymentBizPlanSettleDto bizPlanSettleDto : financeBaseDto.getCurrentPeriods()) {
			...
			if (!StringUtil.isEmpty(financeBaseDto.getRemark())) {
				...
				RepaymentResource lastOne = financeBaseDto.getRepaymentResources()
						.get(financeBaseDto.getRepaymentResources().size() - 1);
				...
				case "20":
					...
					WithholdingRepaymentLog log = withholdingRepaymentLogService
							.selectById(lastOne.getRepaySourceRefId());
				...
				case "21":
					...
					WithholdingRepaymentLog log1 = withholdingRepaymentLogService
							.selectById(lastOne.getRepaySourceRefId());
				...
				for (RepaymentBizPlanListDetail planListDetail : bizPlanSettleDto.getCurrBizPlanListDto()
						.getBizPlanListDetails()) {
					if (financeBaseDto.getProjFactRepays().containsKey(planId)) {
						if (financeBaseDto.getProjFactRepays().get(planId)
								.containsKey(planListDetail.getPlanDetailId())) {
							List<RepaymentProjFactRepay> list = financeBaseDto.getProjFactRepays().get(planId)
									.get(planListDetail.getPlanDetailId());
							if (list != null && !list.isEmpty()) {
								...
								for (RepaymentProjFactRepay repaymentProjFactRepay : list) {
									...
									for (RepaymentProjFactRepay repaymentProjFactRepay2 : newList) {
										if (repaymentProjFactRepay2.getFeeId()
												.equals(repaymentProjFactRepay.getFeeId())) {
											existSameFee = true;
											repaymentProjFactRepay2.setFactAmount(repaymentProjFactRepay2
													.getFactAmount().add(repaymentProjFactRepay.getFactAmount()));
										}
									}
									if (!existSameFee) {
								...
								if (planListDetail.getFeeId()
										.equals(RepayPlanFeeTypeEnum.OVER_DUE_AMONT_ONLINE.getUuid())) {
									feeDetails.append("线上滞纳金").append(",");
								} else if (planListDetail.getFeeId()
				...
				String remark = MessageFormat.format("{0}{1}{2}元,费用明细:{3}", repayDate, repayWay.toString(),
						factTotalAmount.setScale(2, RoundingMode.HALF_UP), feeDetails.toString());
				if (StringUtil.isEmpty(bizPlanList.getRemark())) {
	...
	private void updateRemarkNew(FinanceSettleBaseDto financeBaseDto) {
		for (RepaymentBizPlanSettleDto bizPlanSettleDto : financeBaseDto.getCurrentPeriods()) {
			...
			remark.append(DateUtil.formatDate("yyyy-MM-dd HH:mm:ss",
					financeBaseDto.getRepaymentConfirmLog().getCreateTime()));
	 ...
	 * @param now
	 * @return
	 * @author 王继光 2018年7月25日 下午5:19:58
	 */
	@Override
	public List<String> curPeriod(RepaymentBizPlanList now) {
		List<RepaymentBizPlan> plan = repaymentBizPlanMapper
				.selectList(new EntityWrapper<RepaymentBizPlan>().eq("business_id", now.getBusinessId()));
		List<String> res = new ArrayList<>();
		for (RepaymentBizPlan repaymentBizPlan : plan) {
			List<RepaymentBizPlanList> selectList = repaymentBizPlanListMapper
					.selectList(new EntityWrapper<RepaymentBizPlanList>().eq("business_id", now.getBusinessId())
			...
			if (CollectionUtils.isEmpty(selectList)) {
				selectList = repaymentBizPlanListMapper.selectList(new EntityWrapper<RepaymentBizPlanList>()
						.eq("business_id", now.getBusinessId()).eq("plan_id", repaymentBizPlan.getPlanId())
						// .ge("due_date", new Date())
	 ...
	 * 
	 * @see
	 * com.hongte.alms.finance.service.FinanceSettleService#getCurrentPeriod(com.
	 * hongte.alms.finance.req.FinanceSettleReq)
	 */
	...
	public List<RepaymentBizPlanList> getCurrenPeroids(com.hongte.alms.base.vo.finance.FinanceSettleReq req) {
		...
		for (RepaymentBizPlan repaymentBizPlan : plan) {
			...
			if (CollectionUtils.isEmpty(selectList)) {
				...
				// // 且为未还款
				// if
				// (!lastBizPlanList.getCurrentStatus().equals(RepayCurrentStatusEnums.已还款.toString()))
				// {
			...
			if (CollectionUtils.isEmpty(selectList)) {
				if (!StringUtil.isEmpty(req.getPlanId())) {
					logger.error("找不到此业务还款计划的当前期 RepaymentBizPlanList  businessId:" + req.getBusinessId()
							+ "     planId:" + repaymentBizPlan.getPlanId());
	...
	public List<RepaymentBizPlanSettleDto> getCurrentPeriod(FinanceSettleReq req,
		...
		for (RepaymentBizPlan repaymentBizPlan : plan) {
			...
			for (RepaymentBizPlanList curRepaymentBizPlanList : cuRepaymentBizPlanLists) {
				...
				List<RepaymentBizPlanListDetail> planListDetails = repaymentBizPlanListDetailMapper
						.selectList(new EntityWrapper<RepaymentBizPlanListDetail>().eq("plan_list_id",
								curRepaymentBizPlanList.getPlanListId()));
				...
				for (RepaymentBizPlanList bizPlanList : BizPlanLists) {
					...
					if (bizPlanList.getPeriod() < curRepaymentBizPlanList.getPeriod()
							&& bizPlanList.getPlanId().equals(curRepaymentBizPlanList.getPlanId())) {
			...
			for (RepaymentProjPlan repaymentProjPlan : projPlans) {
				...
				List<RepaymentProjPlanList> curprojPlanLists = repaymentProjPlanListMapper.selectList(
						new EntityWrapper<RepaymentProjPlanList>().eq("proj_plan_id", repaymentProjPlan.getProjPlanId())
								.eq("plan_list_id", cuRepaymentBizPlanLists.get(0).getPlanListId()));

				// 如果找不到这个标的还款计划当前期
				if (CollectionUtils.isEmpty(curprojPlanLists)) {
					logger.error(
							"找不到这个标的还款计划当前期 RepaymentProjPlanList  proj_plan_id:" + repaymentProjPlan.getProjPlanId()
									+ "     plan_list_id:" + cuRepaymentBizPlanLists.get(0).getPlanListId());
					throw new SettleRepaymentExcepiton("找不到这个标的还款计划当前期",
							ExceptionCodeEnum.NO_BIZ_PLAN_LIST.getValue().toString());
				} else if (curprojPlanLists.size() > 1) {
					logger.error("这个标有两个以上当前期 RepaymentProjPlanList  proj_plan_id:" + repaymentProjPlan.getProjPlanId()
							+ "     plan_list_id:" + cuRepaymentBizPlanLists.get(0).getPlanListId());
					throw new SettleRepaymentExcepiton("这个标有两个以上当前期",
							ExceptionCodeEnum.NO_BIZ_PLAN_LIST.getValue().toString());
				}
				...
				List<RepaymentProjPlanList> projPlanLists = repaymentProjPlanListMapper
						.selectList(new EntityWrapper<RepaymentProjPlanList>()
								.eq("proj_plan_id", repaymentProjPlan.getProjPlanId()).orderBy("due_date"));
				...
				for (RepaymentProjPlanList repaymentProjPlanList : projPlanLists) {
					...
					if (repaymentProjPlanList.getPeriod() > curRepayProjPlanList.getPeriod()
							&& repaymentProjPlanList.getProjPlanId().equals(curRepayProjPlanList.getProjPlanId())) {
		...
		for (RepaymentBizPlanSettleDto bizPlanDto : res) {
			...
			for (RepaymentProjPlanSettleDto repaymentProjPlanDto : projPlanDtos) {
				...
				// 当期结清，应支付的费用明细
				Map<String, PlanListDetailShowPayDto> curShowPayMoney = calcShowPayFeels(curRepaymentProjPlanListDtos,
						null);
				repaymentProjPlanDto.setCurShowPayFeels(curShowPayMoney);

				for (RepaymentProjPlanListDetail detail : repaymentProjPlanDto.getCurrProjPlanListDto()
						.getProjPlanListDetails()) {
				...
				// 当前期之前期数费用的统计
				Map<String, PlanListDetailShowPayDto> beforeFeels = calcShowPayFeels(
						repaymentProjPlanDto.getBeforeProjPlanListDtos(),
						RepayPlanFeeTypeEnum.OVER_DUE_AMONT_UNDERLINE.getUuid());
				repaymentProjPlanDto.setBeforeFeels(beforeFeels);

				// 当前期之后期数费用的统计(只累加本金)
				Map<String, PlanListDetailShowPayDto> afterFeels = calcShowPayFeels(
						repaymentProjPlanDto.getAfterProjPlanListDtos(), RepayPlanFeeTypeEnum.PRINCIPAL.getUuid());
	...
	private void addToCurShowPay(PlanListDetailShowPayDto showPayDto, PlanListDetailShowPayDto planListDetailShowPayDto,
			Map<String, PlanListDetailShowPayDto> curShowPayMoney) {
		if (showPayDto == null) {
	...
	private Map<String, PlanListDetailShowPayDto> calcShowPayFeels(List<RepaymentProjPlanListDto> projPlanLists,
			String feelId) {
		// 当前期之前期数费用的统计
	...
	private void calcShowPayFeel(RepaymentProjPlanListDto bforePlanDto, String feelId,
			Map<String, PlanListDetailShowPayDto> beforeFeels) {
		List<RepaymentProjPlanListDetailDto> repaymentProjPlanListDetailDtos = bforePlanDto
		...
		for (RepaymentProjPlanListDetailDto projPlanListDetailDto : repaymentProjPlanListDetailDtos) {
			...
			if (!CollectionUtils.isEmpty(factRepayList)) {
				...
				if (showPayMoney.compareTo(new BigDecimal("0")) > 0) {
					...
					planListDetailShowPayDto
							.setShowPayMoney(planListDetailShowPayDto.getShowPayMoney().add(showPayMoney));
				}
			} else {
				PlanListDetailShowPayDto planListDetailShowPayDto = beforeFeels.get(detail1.getFeeId());
				if (planListDetailShowPayDto == null) {
				...
				planListDetailShowPayDto
						.setShowPayMoney(planListDetailShowPayDto.getShowPayMoney().add(detail1.getProjPlanAmount()));
			}
		}
	}
	...
	private RepaymentProjPlanListDto creatProjPlanListDto(RepaymentProjPlanList repaymentProjPlanList,
		...
		for (RepaymentProjPlanListDetail projPlanListDetail : projPlanListDetails) {
			...
			List<RepaymentProjFactRepay> factRepayList = repaymentProjFactRepayMapper
					.selectList(new EntityWrapper<RepaymentProjFactRepay>()
							.eq("proj_plan_detail_id", projPlanListDetail.getProjPlanDetailId()).eq("is_cancelled", 0));
