	...
	public Result batchShareProfit(@RequestBody String businessIds){
		...
		for(int i=0;i<array.length;i++) {
			...
			for(WithholdingRepaymentLog log:logs) {
				int count=repaymentResourceService.selectCount(new EntityWrapper<RepaymentResource>().eq("repay_source_ref_id", log.getLogId()).eq("org_business_id", businessId).eq("isCancelled", 0));
				if(count>0) {
					continue;
				}
			    try {
