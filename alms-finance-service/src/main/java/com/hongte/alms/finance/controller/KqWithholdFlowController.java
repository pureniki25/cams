...
@Api(tags = "KqWithholdFlowController", description = "快钱代扣流水", hidden = true)
public class KqWithholdFlowController {
     ...
     * @return
     */
    @ApiOperation(value = "快钱代扣流水列表")
    @GetMapping("/getKqWithholdFlowPageList")
    @ResponseBody
    // public PageResult<List<YbWithholdFlowVo>>
    // getYbWithholdFlowPageList(WithholdFlowReq withholdFlowReq) {
    public PageResult<List<WithholdingFlowRecord>> getKqWithholdFlowPageList(WithholdFlowReq withholdFlowReq) {
        LOGGER.info("====>>>>>快钱代扣流水列表[{}]", JSON.toJSONString(withholdFlowReq));
        try {
            ...
            ew.eq("withholding_platform", PlatformEnum.KQ_FORM.getValue());
