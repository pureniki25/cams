                    ...
                    },
                    {
                        ...
                        render:(h,params)=>{
                        	if(params.row.balanceName.search("合计") != -1 ) {
                        		...
                        		},params.row.balanceName)
                        	}else if(params.row.balanceName.search("总计") != -1){
                        		return h('span',{
        	            			style:{
        	            			 'font-weight': 'bold',
                        		...
                        		return h('span',{},params.row.balanceName)  
                        	}
                        }
                        ...
                        render:(h,params)=>{
                        	if(params.row.balanceName.search("合计") != -1 ) {
                        		...
                        		return h('span',{},params.row.firstAmount)  
                        	}
                        }
                    }, 
                  {
                        ...
                        render:(h,params)=>{
                        	if(params.row.balanceName.search("合计") != -1 ) {
                        		...
                        		return h('span',{},params.row.restAmount)  
                        	}
                        }
                    }
                ],
...
let methods = {
    ...
    exportData() {
        ...
        ExportForm.action = basePath + "subjectRestDat/exportBalance";
        ExportForm.target = "iframe";
        addInput(ExportForm, "text", "companyName", vm.searchForm.companyName);
        addInput(ExportForm, "text", "openBeginTime", vm.searchForm.beginDate);
        addInput(ExportForm, "text", "openEndTime", vm.searchForm.endDate);
    ...
    searchData() {
        ...
        axios.post(basePath + 'subjectRestDat/selectBalace', this.searchForm).then(res => {
            self.table.loading = false;
            if (!!res.data && res.data.code == 0) {
                self.table.data = res.data.data;
                self.table.total = res.data.count;
