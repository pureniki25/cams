...
window.layinit(function (htConfig) {
    ...
    vm = new Vue({
        ...
        data:  {
            ...
            banks: [],
        	upload: {
        			url: basePath + 'customerDat/importCustomerFlowExcel',
        			headers: {
...
uploadSuccess(response) {
    ...
    deleteAll(){
       ...
       this.$Modal.confirm({
           ...
           content: '确定要删除吗?',
           onOk: () => {
               axios.post(basePath + 'customerDat/delete', vm.selectIds)
               .then(res => {debugger
                   if (!!res.data && res.data.code == '1') {
    ...
    exportData() {debugger
        ...
        ExportForm.action = basePath+"customerDat/export";
        ExportForm.target = "iframe";

        addInput(ExportForm, "text", "customerName", vm.searchForm.condition.LIKE_customer_name);
        addInput(ExportForm, "text", "customerCode", vm.searchForm.condition.LIKE_customer_code); 
...
// self.loading =  false;
    ...
    if(vm.searchForm.condition.LE_open_date!='') {
        ...
        axios.post(basePath + 'customerDat/search', this.searchForm).then(res => {
            self.table.loading = false;
            if (!!res.data && res.data.code == 0) {
                self.table.data = res.data.data;
                self.table.total = res.data.count;
