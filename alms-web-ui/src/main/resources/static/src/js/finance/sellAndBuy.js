...
uploadSuccess(response) {
    ...
    exportData() {debugger
        ...
        if(vm.searchForm.condition.EQ_fee_type==''){
        	vm.searchForm.condition.IN_fee_type="5,6";
            addInput(ExportForm, "text", "feeType", vm.searchForm.condition.IN_fee_type);
        }else{
        	addInput(ExportForm, "text", "feeType", vm.searchForm.condition.EQ_fee_type);
        }
        ExportForm.submit();
    ...
    if(vm.searchForm.condition.EQ_fee_type==''){
        ...
        axios.post(basePath + 'FeeDatController/sellAndBuySearch', this.searchForm).then(res => {
