...
window.layinit(function (htConfig) {
    ...
    vm = new Vue({
    	...
    	uploadCertify: {
    			url: coreBasePath + 'InvoiceController/importBuyCertifyFlowExcel',
    			headers: {
    			 Authorization: axios.defaults.headers.common['Authorization'],
    			},
      },
...
generateByInvoice(){
      ...
      this.$Modal.confirm({
          ...
          content: '确定生成凭证吗?',
          onOk: () => {
              axios.post(basePath + 'InvoiceController/generateBuyByInvoice', vm.selectIds)
              .then(res => {debugger
                  if (!!res.data && res.data.code == '1') {
  ...
  generateByCompany(){
	        ...
	        this.$Modal.confirm({
	            ...
	            content: '确定生成凭证吗?',
	            onOk: () => {
	                axios.post(basePath + 'InvoiceController/generateBuyByCompany', vm.selectIds)
	                .then(res => {debugger
	                    if (!!res.data && res.data.code == '1') {
