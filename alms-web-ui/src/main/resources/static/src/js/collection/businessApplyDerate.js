...
//////////////   -------------   基础信息  开始 --------------------//////////////////////
            baseInfoForm:{
                ...
                platYearRate:0,
                gurrenteeYearRate:0,
                monthServicePenalty:0,
                platServicePenalty:0,
                principlePenalty:0,
                businessBorrowMoney:0
            },
...
//////////////   -------------   申请减免信息  开始 --------------------//////////////////////
            ...
            applyInfoForm:{
                ...
                title:'',                    //减免标题
                isSettle:'',                //是否结清标志位
                isSettleFlage:'结清减免',           //是否结清界面显示标志位
                feeId:'test',
                isSettleFlag:true,
                generalReturnRate:'',   //综合收益率
                planRepayTime:'' ,          //计划还款日期

                principlePenalty:''  ,          //本金违约金    baseInfoForm.principlePenalty
                platServicePenalty:''   ,         //平台服务费违约金  baseInfoForm.platServicePenalty
                monthServicePenalty:''            //分公司服务费违约金  baseInfoForm.monthServicePenalty
            },
            //后台返回的初始的减免信息
            initalApplyInfo:{},
...
var getTotalShouldPay = function () {
	...
	if(vm.applyInfoForm.isSettleFlage=='月还减免'){
		...
		vm.baseInfoForm.totalBorrowAmount=vm.baseInfoForm.remianderPrincipal+
            vm.baseInfoForm.noSettleNeedPayInterest+
            vm.baseInfoForm.noSettleNeedPayService+
            vm.baseInfoForm.needPayPlatform
		    +vm.baseInfoForm.principlePenalty+
            vm.baseInfoForm.platServicePenalty+
            vm.baseInfoForm.monthServicePenalty+
            (vm.baseInfoForm.doorFee==null?0:vm.baseInfoForm.doorFee*1)
        ;
	}
...
///////////计算逾期收益率
var getOverReturnRate = function () {
	...
	if( vm.applyInfoForm.isSettleFlage=='月还减免'){
		if(vm.baseInfoForm.noSettleOffOverAmount-derateMoney<0){
			...
			overDueYearRate=((vm.baseInfoForm.noSettleOffOverAmount-derateMoney)/vm.baseInfoForm.noSettleOffOverAmount*0.36).toFixed(2);
		}
		...
		if(vm.baseInfoForm.settleOffOverAmount-derateMoney<0){
			...
			overDueYearRate=((vm.baseInfoForm.settleOffOverAmount-derateMoney)/vm.baseInfoForm.settleOffOverAmount*0.36).toFixed(2);
		}
		
	}
	var rate=(Number(overDueYearRate)+Number(interestRate)+Number(companyYearRate)+Number(platYearRate)+Number(gurrenteeYearRate)).toFixed(2);
...
///////////车贷:获取合同期外逾期利息
//var getOutsideInterest = function () {
//
...
////////// --------------  基本信息  赋值  开始---------------//////////
                ...
                //默认加载结清的数据
                  ...
                  if(res.data.data.applyList != null && res.data.data.applyList.length>0){
                      vm.baseInfoForm.doorFee=res.data.data.applyList[0].doorFee;
                  }else {
    ...
    if(processStatus == PROCESS_STATUS_NEW || processStatus == PROCESS_STATUS_START || pResult == 3){
        vm.$refs['applyInfoForm'].validate((valid) => {
            if (valid) {
                ...
                vm.applyInfoForm.principlePenalty = vm.baseInfoForm.principlePenalty;
                vm.applyInfoForm.platServicePenalty = vm.baseInfoForm.platServicePenalty;
                vm.applyInfoForm.monthServicePenalty = vm.baseInfoForm.monthServicePenalty;
