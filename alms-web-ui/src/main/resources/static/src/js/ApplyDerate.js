...
////////////////////  ---------------  流程审批信息 赋初始值 结束 -------------------////////////
 ...
 ////////// --------------  界面显示控制   结束---------------//////////
                ...
                //控制界面显示
                if(processStatus == PROCESS_STATUS_NEW || processStatus == PROCESS_STATUS_START){debugger//流程状态为新建或开始时
                    //设置申请信息可编辑
 ...
 * 
 */
var getAfterApplyRepayAmount= function () {debugger
...
if(vm.applyInfoForm.isSettleFlage=='结清减免'){
  ...
  var self = this;   
  var url = financePath +"CalApplyDerateController/calApplyDerateDetail?afterId="+afterId+"&businessId=" + businessId+"&applyDerateAmount="+vm.applyInfoForm.applyDerateMoney+"&pListId="+crpId+"&isSettle="+vm.applyInfoForm.isSettle
        ...
        vm.$refs['applyInfoForm'].validate((valid) => {debugger
            if (valid) {
                ...
                $.ajax({
		               ...
		               data: JSON.stringify({"applyData":[vm.applyInfoForm],"reqRegFiles":vm.reqRegFiles,"doorFee":vm.baseInfoForm.doorFee
		               }),
