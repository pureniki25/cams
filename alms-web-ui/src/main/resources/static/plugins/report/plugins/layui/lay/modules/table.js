...
layui.define(['laytpl', 'laypage', 'layer', 'form', 'laydate'], function (exports) {
    "use strict";
    ...
    Class.prototype.render = function () {
        ...
        //生成替代元素
        if (hasRender[0])
            hasRender.remove(); //如果已经渲染，则Rerender
        othis.after(reElem);
    ...
    Class.prototype.setArea = function () {
        ...
        if (cntrWidth > countWidth && autoColNums) {
        ...
        //高度铺满：full-差距值
        if (options.height && /^full(\/\d+)?(-\d+)?$/.test(options.height)) {
            options.height = options.height.replace('full', _WIN.height());
            var Fn = Function;
            options.height = (new Fn('return ' + options.height)());
        }
    };
        ...
        if (options.url) { //Ajax请求
            ...
            $.ajax({
                ...
                success: function (res) {
                    if (that.resData(res, response.statusName) != response.statusCode) {
                        that.renderForm();
                        that.layMain.html('<div class="' + NONE + '">' + (that.resData(res, response.msgName) || '返回的数据状态异常') + '</div>');
                    } else {
                        that.renderData(res, curr, that.resData(res, response.countName));
                        sort();
                    ...
                    if (typeof options.done === 'function')
                        options.done(res, curr, that.resData(res, response.countName));
                },
                error: function (e, m) {
            ...
            that.resSetData(res, response.dataName, options.data.concat().splice(startLimit, options.limit));
            //data = options.data.concat().splice(startLimit, options.limit);
            that.resSetData(res, response.countName, options.data.length);
    ...
    Class.prototype.renderData = function (res, curr, count, sort) {
        var that = this,
            options = that.config,
            data = that.resData(res, options.response.dataName) || [],
            trs = [],
            trs_fixed = [],
            trs_fixed_r = [],
    ...
    Class.prototype.sort = function (th, type, pull, formEvent) {
        ...
        var x = that.resData(res, options.response.dataName);
        x = thisData;
    ...
    Class.prototype.scrollPatch = function () {
        ...
        // 增加判断条件layMainTable.outerWidth()>0，因为在表格重复渲染时，操作列显示出现异常，by tanrq 2018/1/13
        if (layMainTable.outerWidth() > 0 && that.autoColNums && outWidth < 5 && !that.scrollPatchWStatus) {
    ...
    Class.prototype.events = function () {
        ...
        that.elem.on('click', 'input[name="layTableRadio"]+', function () {
            ...
            if (table.cache[that.key]) {
                for (var i = 0; i < table.cache[that.key].length; i++) {
                    if (table.cache[that.key][i].LAY_CHECKED)
                        delete table.cache[that.key][i].LAY_CHECKED;
                }
            }
            that.setCheckData(index, checked);
        ...
        that.layBody.on('click', 'td', function () {
            ...
            //显示编辑表单
            if (editType) {
                if (editType === 'SQL') { //资源选择触发按钮
                } else if (editType === 'TEXT') { //选择框
                } else if (editType === 'TIME') { //时间选择框
                } else { //输入框
                    var input = $('<input class="layui-input ' + ELEM_EDIT + '">');
                    input[0].value = othis.data('content') || elemCell.text();
                    if (!othis.find('.' + ELEM_EDIT)[0])
                        othis.append(input);
                    input.focus();
                }
                return;
            ...
            //如果出现省略，则可查看更多
            if (elemCell.find('.layui-form-switch,.layui-form-checkbox,.layui-form-radio')[0]) return; //限制不出现更多（暂时）
    ...
    table.init = function (filter, settings) {
        ...
        elemTable.each(function () {
            ...
            try {
                var Fn = Function;
                tableData = new Fn('return ' + tableData)();
            } catch (e) {
            ...
            othis.find('thead>tr').each(function (i) {
                ...
                $(this).children().each(function (ii) {
                    ...
                    try {
                        var Fn = Function;
                        itemData = new Fn('return ' + itemData)();
                    } catch (e) {
    ...
    //add by yaojh 2018/3/14 表格数据检索条件清除
    table.clearWhere = function (id) {
        var config = thisTable.config[id];
        if (config)
            delete config.where;
    };

    //add by yaojh 2018/3/14 扩展表网络数据获取自动字段的能力，运行复杂数据结构的内容定位
    Class.prototype.resData = function (res, id) {
        var name = id.split('.');
        var data;
        if (0 < name.length)
            data = res[name[0]];
    ...
    Class.prototype.resSetData = function (res, id, value) {
        var name = id.split('.');
        if (1 == name.length) {
            res[name[0]] = value;
            ...
            res[name[0]][name[1]][name[2]][name[3]] = value;
        } else if (5 == name.length) {
            res[name[0]][name[1]][name[2]][name[3]][name[4]] = value;
        }
    };
