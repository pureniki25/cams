 ...
 * LICENSE:MIT
 */
layui.define(['jquery', 'element', 'nprogress', 'utils', 'ht_config', 'handsontable'], function (exports) {
    var $ = layui.jquery,
    ...
    var _tab = {
        ...
        createTabDom: function () {
            ...
            var tpl = [
                    ...
                    '<div class="layui-tab-item layadmin-tabsbody-item layui-show" lay-item-id="-1">{{content}}</div>',
                    '</div>',
                    '</div>'
                ],
                _htm = tpl.join('');
                        ...
                        case 'refresh': //刷新
                            switch (_config.renderType) {
                                case renderType.page:
                                    ...
                                    if (1 == that.getUrlParam(url, 'gateway')) {
                                        url = ht_config._basePath + url.substr(1);
                                    }
                                    that._content.children('div[lay-item-id=' + layId + ']')
                                        .html(that.getBodyContent(url + (url.indexOf("?") >= 0 ? '&' : '?') + 'v=' + new Date().getTime(), function () {
        ...
        getUrlParam: function (url, param) {
            return utils.getUrlParam(url, param);
        },
        /**
         * 监听浏览器窗口改变事件
         */
        ...
        tabAdd: function (options) {
            ...
            switch (_config.renderType) {
                case renderType.page:
                    if (that.getUrlParam(url, 'code')) {
                        ht_config.dataPool.RP['code'] = that.getUrlParam(url, 'code');
                    }
                    if (1 == that.getUrlParam(url, 'gateway')) {
                        url = ht_config._basePath + url.substr(1);
                    }
                    var content = that.getBodyContent(url + (url.indexOf("?") >= 0 ? '&' : '?') + 'v=' + new Date().getTime());
                    content = content.replace('src = \'/', 'src = \'' + ht_config._basePath);
                    contentHtm = contentHtm.replace('{{content}}', content);
            ...
            if (_config.onSwitch) {
                element.on('tab(' + that._filter + ')', function (data) {
                    // Add By YJH For 报表页面里handsontable自适应能力
                    data.elem.find('div[table]').each(function () {
                        if ($(this).handsontable('getInstance'))
