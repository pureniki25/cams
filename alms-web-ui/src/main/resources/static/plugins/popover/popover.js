    ...
    *       popover.setContent      - Fired after popover's contenet is set.
    ...
    var methods = {
        _init: function(options, popover, e) {
            ...
            // Lee
            if (typeof(options.onBefore) !== 'undefined') {
        ...
        _optionsPopoverInit: function (options) {
            var popover = new OptionsPopover(this);
            methods._init(options, popover, this);
            return popover;
        },
        ...
        addMenu: function (menu) {
            // 李钊鸿再此修改加入了tableid
            Popover.addMenu(menu.id, menu.title, menu.contents, menu.tableid);
        },
...
OptionsPopover.prototype.setData = function (data) {
    ...
    for (i = 0; i < contArray.length; i++) {
        ...
        var $a = $("<a" + menuUrl + menuId + " class='popoverContentRow" + popoverEvent + lastElement + "'>" +
                     contArray[i].name +
                 "</a>");


        (function (j, ele) {
            data.contents[j].click &&  $a.click(function () {
                var that = ele.superConstructor.currentTarget
                var index = that.parents('tr').index();
                var tableid = data.tableid;
                var json = (tableid && layui.table && layui.table.cache && layui.table.cache[tableid][index]) || {};
                data.contents[j].click(that, json)
            }) 
